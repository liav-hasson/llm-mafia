...
func TestIsTie(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "empty votes is not a tie",
			votes:    map[string]string{},
			expected: false,
		},
		{
			name:     "single vote is not a tie",
			votes:    map[string]string{"voter1": "target1"},
			expected: false,
		},
		{
			name:     "clear winner is not a tie",
			votes:    map[string]string{"voter1": "target1", "voter2": "target1", "voter3": "target2"},
			expected: false,
		},
		{
		...
		{
			name: "tie at top with others below",
			votes: map[string]string{
				"voter1": "target1",
				"voter2": "target1",
				"voter3": "target2",
				"voter4": "target2",
				"voter5": "target3", // target3 has only 1 vote, not in tie
			},
			expected: true, // target1 and target2 are tied at 2 votes each
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := IsTie(tt.votes)
